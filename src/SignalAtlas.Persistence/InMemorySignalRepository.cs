...
/// Optionally seeded with one signal so the UI has data with no ingestion. Bounded + thread-safe
/// (SPEC NFR-C3): the live pipeline appends per block from its own thread while REST handlers read
/// concurrently, and continuous streaming must not grow unboundedly. Reads return the NEWEST signals
/// first — returning the oldest/seed left the UI showing stale "demo" data even while live
/// classifications streamed in.
...
/// </summary>
public sealed class InMemorySignalRepository : ISignalRepository, ISignalWriter, IDemoSeedStore, ITransientStore
{
    ...
    public InMemorySignalRepository(bool seedDemo = false)
    {
        if (!seedDemo) return;
    ...
    /// <summary>Drop all signals — used on retune (SPEC §8.1) so a new band starts clean. Devices are
