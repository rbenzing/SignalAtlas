...
| §8.6 Geospatial | ✅ | `GeolocationEngine` (power-weighted centroid + honest uncertainty + heatmap). Wired as the **`GET /map/heatmap`** consumer (audit #14); the RF Map renders emitter uncertainty rings and a **server-fed observation-density heatmap overlay** (`/map/heatmap` → `heatmapCellsToGeoJSON`, weighted by per-cell observation count). Emitter positions are still set from the latest observation in correlation — the multi-obs centroid path is a **staged seam** pending persisted per-emitter sighting history (landmine #8). |
