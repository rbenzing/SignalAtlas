...
- **#14 — resolved (2026-07-25):** `GeolocationEngine` is now DI-wired with a real runtime consumer — **`GET /map/heatmap`** (bucketing positioned observations; also closes a §9.2 endpoint gap). Its multi-obs emitter-**centroid** path and the entire `SignalAtlas.Behavior` project (`BehaviorEngine` / `BehaviorPredictor` / `PredictedAnomalyDetector`, §8.7/§8.11) are **explicitly marked staged seams** in SPEC §19: built + unit-tested, but runtime activation is gated on persisting per-emitter sighting history (#24 / landmine #8), a tracked milestone — not a quick wire-up. Green build ≠ silently-shipped: the dormant capability is now documented as staged rather than implied complete.
